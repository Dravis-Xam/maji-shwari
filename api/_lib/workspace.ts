export type WorkspacePayload = {
  source: 'neon' | 'demo'
  releases: Array<{ projectId: string; projectName: string; amount: string; status: 'Approved' | 'Pending approval'; milestone: string }>
...
export const demoWorkspace: WorkspacePayload = {
  ...
  rules: [
    { title: 'Unique reporters', detail: 'Count one confirmation per reporter, project, and status.' },
    { title: 'Threshold', detail: 'A project reaches verified after its configured confirmation count.' },
    { title: 'Audit flags', detail: 'DELAYED, INCOMPLETE, and PROBLEM reports create an audit flag.' },
    { title: 'Evidence format', detail: 'Messages must use PROJECT-CODE STATUS, such as BHR-042 DONE.' },
  ],
  activity: [
    ...
    { event: 'BHR-042 reached verification threshold', detail: 'Verification engine', age: '3 hours ago' },
    { event: 'Daily reconciliation scheduled for 02:00 UTC', detail: 'System scheduler', age: '4 hours ago' },
  ],
}
