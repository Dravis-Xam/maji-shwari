...
function App() {
  ...
  if (!user) return <div className="auth-screen"><div className="auth-card"><span className="brand-mark"><Sprout size={19} /></span><p className="eyebrow">KENYA CLIMATE DESK</p><h1>Sign in to MajiShwari</h1><p>Use your approved Google account. No separate MajiShwari password is stored.</p><button className="google-button" onClick={() => { window.location.href = '/api/auth/google/start' }}><span>G</span> Continue with Google</button>{import.meta.env.DEV && <details className="dev-login"><summary>Local preview login</summary><p>Local preview uses a fixed community role. Production roles come from Google allowlists.</p><input className="auth-input" value={loginName} onChange={(event) => setLoginName(event.target.value)} placeholder="Your name" /><button className="secondary-button auth-button" disabled={loginName.trim().length < 2} onClick={handleLogin}>Preview locally</button></details>}</div></div>
