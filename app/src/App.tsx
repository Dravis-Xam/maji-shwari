...
const demoProjects = [
  {
    ...
    amount: 'KES 4.8M',
    color: 'green',
    ownerId: 'local:jane-mwangi',
    lifecycleState: 'In progress' as LifecycleState,
  },
  {
    id: 'WTR-117',
    ...
    amount: 'KES 7.2M',
    color: 'amber',
    ownerId: 'local:peter-otieno',
    lifecycleState: 'Submitted' as LifecycleState,
  },
  {
    id: 'FRM-089',
    ...
    amount: 'KES 3.1M',
    color: 'green',
    ownerId: 'local:grace-wanjiru',
    lifecycleState: 'Completed' as LifecycleState,
  },
]
const demoReports = [
  {
...
type User = { sub?: string; name: string; email?: string; role: Role | 'pending' }

type LifecycleState = 'Draft' | 'Submitted' | 'In progress' | 'Completed'
const LIFECYCLE_TRANSITIONS: Record<LifecycleState, { to: LifecycleState; label: string }[]> = {
...
// Community/government users may move a project they own through its
// lifecycle; government has oversight and may move any project; donors
// can never trigger a transition. Mirrors the server-side check in
// PATCH /api/projects.
function canManageLifecycle(user: User | null, project: { ownerId?: string }) {
  if (!user || user.role === 'pending' || user.role === 'donor') return false
  if (user.role === 'government') return true
  return Boolean(user.sub) && project.ownerId === user.sub
}
...
function App() {
  ...
  useEffect(() => {
    fetch('/api/auth/me')
      ...
      .then((payload) => {
        ...
        // A single-approved-role account skips the picker entirely (see
        // /api/auth/google), so it never runs through finishRoleOnboarding
        // and would otherwise get zero first-login onboarding. The
        // ?onboarding=welcome marker on that redirect gives us the same
        // "just signed in" signal here instead.
        const justSignedIn =
          new URLSearchParams(window.location.search).get('onboarding') === 'welcome'
        if (justSignedIn && payload.user && payload.user.role !== 'pending') {
  ...
  const handleLifecycleTransition = async (projectId: string, to: LifecycleState) => {
    if (lifecycleBusy) return
    setLifecycleBusy(projectId)
    try {
      const response = await fetch('/api/projects', {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ id: projectId, lifecycleState: to }),
      })
      const payload = await response.json()
      if (!response.ok) {
        setNotice(payload.error ?? 'Project status could not be updated.')
        return
      }
      setProjects((current) =>
        current.map((project) =>
          project.id === projectId ? { ...project, lifecycleState: to } : project,
        ),
      )
      setNotice(`${projectId} moved to ${to}.`)
      window.setTimeout(() => setNotice(''), 4500)
    } catch {
  ...
  const renderLifecycleControl = (project: (typeof projects)[number]) => {
    const state = (project.lifecycleState ?? 'Draft') as LifecycleState
    const transitions = LIFECYCLE_TRANSITIONS[state] ?? []
    const manageable = canManageLifecycle(user, project)
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-start' }}>
        <small className="muted">{state}</small>
        {manageable &&
          transitions.map((transition) => (
            <button
              key={transition.to}
              className="text-button"
              disabled={lifecycleBusy === project.id}
              onClick={() => handleLifecycleTransition(project.id, transition.to)}
            >
              {lifecycleBusy === project.id ? 'Updating…' : transition.label}
            </button>
          ))}
      </div>
    )
                  ...
                  {projects.map((project) => (
                    <div className="project-row" key={project.id}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        ...
                        {renderLifecycleControl(project)}
                      </div>
