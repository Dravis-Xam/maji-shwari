...
function App() {
  ...
  if (!user) return <div className="auth-screen"><div className="auth-card"><span className="brand-mark"><Sprout size={19} /></span><p className="eyebrow">KENYA CLIMATE DESK</p><h1>Sign in to MajiShwari</h1><p>Use your role to access verified project and funding workflows.</p><input className="auth-input" value={loginName} onChange={(event) => setLoginName(event.target.value)} placeholder="Your name" /><select className="auth-input" value={loginRole} onChange={(event) => setLoginRole(event.target.value as Role)}><option value="community">Community member</option><option value="government">Government official</option><option value="donor">Donor</option></select><button className="primary-button auth-button" disabled={loginName.trim().length < 2} onClick={handleLogin}>Continue securely</button></div></div>
